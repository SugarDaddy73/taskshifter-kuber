...
export function EditTaskDialog({ 
  ...
  const [estimatedPoints, setEstimatedPoints] = useState(
    task.estimatedPoints ? task.estimatedPoints.toString() : ''
  );
  const [assigneeId, setAssigneeId] = useState(task.assigneeId || 'unassigned');
  const [columnId, setColumnId] = useState(task.columnId || 'unassigned');
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setEstimatedPoints(task.estimatedPoints ? task.estimatedPoints.toString() : '');
      setAssigneeId(task.assigneeId || 'unassigned');
      setColumnId(task.columnId || 'unassigned');
    }
  }, [isOpen, task]);
  ...
  const handleSave = async () => {
    ...
    setIsLoading(true);
    try {
      // Prepare data for update - конвертуємо спеціальні значення назад в undefined
      const updateData = {
        title: title.trim(),
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl gap-4">
        <DialogTitle className="text-xl flex justify-center">Edit Task</DialogTitle>
        <Separator />
                ...
                <SelectTrigger>
                  <SelectValue>
                    {columnId !== 'unassigned' ? (
                      ...
                      'No column | Undistributed'
                    )}
                  </SelectValue>
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="unassigned">No column | Undistributed</SelectItem>
